...
export default class EmployeeController extends Controller implements CRUD {
    ...
    async create(): Promise<Employee> {
		...
		try {
			const values = [this.employee.firstName, this.employee.lastName, this.employee.roleId, this.employee.managerId];
    ...
    async update(fields: Partial<EmployeeUpdatable>): Promise<Employee> {
		if (!this.employee) throw new Error('Employee is not defined');
		try {
			const values = [fields.firstName ?? this.employee.firstName, fields.lastName ?? this.employee.lastName, fields.roleId ?? this.employee.roleId, fields.managerId ?? this.employee.managerId, this.employee.id];
			const query = 'UPDATE employee SET first_name = COALESCE($1, first_name), last_name = COALESCE($2, last_name), role_id = COALESCE($3, role_id), manager_id = COALESCE($4, manager_id) WHERE id=$5 RETURNING *;';
