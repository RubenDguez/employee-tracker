...
export default class EmployeeController extends Controller implements CRUD {
	...
	async update(fields: Partial<EmployeeUpdatable>): Promise<Employee> {
		...
		try {
			...
			const updateTransaction = 'UPDATE employee_transactions SET updated_at = COALESCE(CURRENT_TIMESTAMP, updated_at), updated_by = COALESCE($1, updated_by) WHERE employee_id = $2;';
